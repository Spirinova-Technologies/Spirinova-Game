...
public class DelphiGuardianUniversal : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha1) && gm.selectedUnit != null && gm.selectedUnit.name == "Delphi, Guardian Universal(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.curacionAvanzadaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
            CuracionAvanzada(gm.selectedUnit);     
        }
        
        if (Input.GetKeyDown(KeyCode.Alpha2) && gm.selectedUnit != null && gm.selectedUnit.name == "Delphi, Guardian Universal(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.posicionDeDefensaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
