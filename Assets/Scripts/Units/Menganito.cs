...
public class Menganito : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha1) && gm.selectedUnit != null && gm.selectedUnit.name == "Menganito(Clone)" && gm.selectedUnit.actionPoints >= 6 && gm.selectedUnit.conquistaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
            Conquista(gm.selectedUnit);
            gm.UpdateStatsPanel();      
        }

        if (Input.GetKeyDown(KeyCode.Alpha2) && gm.selectedUnit != null && gm.selectedUnit.name == "Menganito(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.espadaDeLaMuerteCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
