...
public class DelphiElMitologico : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha1) && gm.selectedUnit != null && gm.selectedUnit.name == "Delphi, el Mitologico(Clone)" && gm.selectedUnit.actionPoints >= 6 && gm.selectedUnit.furiaEternaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
            FuriaEterna(gm.selectedUnit);
        }

        if (Input.GetKeyDown(KeyCode.Alpha2) && gm.selectedUnit != null && gm.selectedUnit.name == "Delphi, el Mitologico(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.curacionAvanzadaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
