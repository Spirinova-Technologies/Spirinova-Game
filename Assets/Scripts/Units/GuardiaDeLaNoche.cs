...
public class GuardiaDeLaNoche : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha2) && gm.selectedUnit != null && gm.selectedUnit.name == "Guardia de la Noche(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.resistenciaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
