...
public class ChiquitiboomBombita : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {   
        if (Input.GetKeyDown(KeyCode.Alpha1) && gm.selectedUnit != null && gm.selectedUnit.name == "Chiquitiboom Bombita(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.bazookaCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
            Bazooka(gm.selectedUnit);  
        }

        if (Input.GetKeyDown(KeyCode.Alpha2) && gm.selectedUnit != null && gm.selectedUnit.name == "Chiquitiboom Bombita(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.disparoExplosivoCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
