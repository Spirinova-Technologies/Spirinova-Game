...
public class GaiaElCaballeroFeroz : MonoBehaviourPunCallbacks
{
    GameMaster gm;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Alpha1) && gm.selectedUnit != null && gm.selectedUnit.name == "Gaia, el Caballero Feroz(Clone)" && gm.selectedUnit.actionPoints >= 4 && gm.selectedUnit.gritoDelDragonCast == false && gm.selectedUnit.transform.position == this.transform.position)
        {
